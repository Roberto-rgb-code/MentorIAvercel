...
interface DiagnosticoProfundoData {
  ...
  fa_situacionFinancieraGeneral: Likert | string;
  ...
  ra_tanSatisfechoRolActual: Likert | string;
  ra_referenteParaEquipo: Likert;
  ra_situacionFinancieraGeneral: Likert | string;
  createdAt: string;
}
...
const ChatbotDiagnostico: React.FC = () => {
    ...
    setMessages([
      {
        ...
        text: (
          <>
            <b>Diagnóstico Profundo Empresarial</b>
            <br />
            Te haré preguntas breves por áreas. Responde con texto o usa los botones cuando aparezcan. Empezamos 👇
          </>
        ),
      },
    ]);
        ...
        ...m,
        {
          ...
          text: (
            <div>
              <div>{s.prompt} *</div>
              <LikertChips
                value={(data[s.field] as Likert) || ""}
                onSelect={(v) => handleLikertAnswer(s.field, v)}
              />
            </div>
          ),
        },
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {s.options.map((op) => (
                  <button
                    key={op.value}
                    className="px-3 py-2 rounded-xl border bg-white hover:bg-indigo-50"
          ...
          ...m,
          {
            ...
            text: (
              <div>
                ...
                <button
                  className="underline"
                  onClick={() => jumpToField(missing[0] as keyof DiagnosticoProfundoData)}
                >
                  completar la primera pendiente
                </button>{" "}
                o{" "}
                <button className="underline" onClick={sendAll}>
            ...
            text: (
              <div>
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  <button className="px-4 py-2 rounded-xl border" onClick={() => setIdx(0)}>
                    Revisar
                  </button>
                </div>
              </div>
  ...
  async function sendAll() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/diagnostico/profundo/analyze`, {
        method: "POST",
      ...
      if (!res.ok) {
        throw new Error((json && (json.error || json.detail)) || "Error al procesar el diagnóstico profundo");
      }
      setAnalysis(json as LLMAnalysisResult);
    ...
    <>
      ...
      {loading && (
        <>
          ...
          `}</style>
          <div className="fixed inset-0 z-50 bg-gray-900/70 backdrop-blur-sm flex flex-col items-center justify-center">
            <div className="spinner" />
            <div className="mt-4 text-white font-semibold">Analizando con IA…</div>
          </div>
        </>
      )}
        ...
        <div ref={viewportRef} className="w-full max-w-5xl bg-white border rounded-2xl shadow p-4 md:p-6 h-[60vh] md:h-[70vh] overflow-y-auto">
          ...
          {!loading && (
            <div className="text-[11px] text-gray-400 mt-2">
              Consejo: puedes usar Enter para enviar rápidamente en preguntas de texto.
            </div>
          )}
          {loading && (
            <div className="mb-3">
